...
| `Enemy/` | Enemy brains + boss | **EnemyAI** (chase/aggro) + **EnemyCombat** (telegraphed attacks) on every regular enemy; CaptainCombat (cycles attack shapes); **FatRatBoss** (self-contained boss state machine — does NOT use EnemyAI/EnemyCombat); BossMinionSpawner; EnemyDeathFade; EnemyXPDrop; AttackShape (shared enum) |
